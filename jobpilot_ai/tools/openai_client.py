...
class OpenAIClient:
    ...
    def _with_retries(self, fn, *, action: str):
        ...
        while True:
            try:
                if attempts:
                    log.info("Retrying %s (attempt %s/%s)", action, attempts + 1, self._retry.max_retries + 1)
                return fn()
                ...
                log.warning("Transient OpenAI error during %s: %s. Sleeping %.2fs", action, type(e).__name__, delay)
                time.sleep(delay)
                attempts += 1
